<?php
/**
 * Created by PhpStorm.
 * User: Sile
 * Date: 1/29/2019
 * Time: 11:55 PM
 */

return [
    'product' => [
        'related_products' => 'Povezani proizvodi',
        'description' => 'Opis',
        'details' => 'Detalji',
        'reviews' => 'Utisci',

        'in_stock' => 'Na stanju',
        'not_in_stock' => 'Nema na stanju',

        'reviews_status' => 'utisak(a) | Dodaj svoj utisak',
        'quantity' => 'kol',
        'brand' => 'Brend',
        'category' => 'Kategorija',
        'share' => 'Podeli',

        'rating' => [
            'average_rating' => 'Srednja ocena',
        ],
        'comments' => [
            'no_comment' => 'Nema komentara! Budite prvi koji ćete da komentarišete!',
        ],
        'comment_form' => [
            'your_rating' => 'Vaša ocena',
            'placeholder' => 'Ostavite komentar',
            'button' => 'Pošalji',
            'please' => 'Molimo Vas',
            'login_link' => 'ulogujte se',
            'rest' => 'kako biste ostavili komentar!',
        ],
    ],

    'home' => [
        'laptop_collection' => '<h3>Laptop<br>kolekcija</h3>',
        'accessories_collection' => '<h3>Accessories<br>kolekcija</h3>',
        'cameras_collection' => '<h3>Kolekcija<br>kamera</h3>',
        'featured' => 'Istaknuti',
        'new_products' => 'Novi proizvodi',
        'top_selling' => 'Najprodavaniji',
        'might_also_like' => 'Možda vam se svidi',
        'discount' => 'Sa popustom',

        'desc1' => '<h2 class="text-uppercase">Nove akcije</h2>',
        'desc2' => '<p>Nova kolekcija sa popustom od 50%</p>',
        'button' => 'Kupite',
        'button2' => 'Kupite',
    ],

    'cart' => [
        'title' => 'Korpa',
        'your_shopping_cart' => 'Vaša korpa',
    ],
    'wish_list' => [
        'title' => 'Lista želja',
        'label' => 'Vaša lista želja',
    ],

    'shop' => [
        'title' => 'Shop',
        'categories' => 'Kategorije',
        'price' => 'Cena',
        'top_selling' => 'Najprodavanije',
        'sort_by' => 'Sortiraj',
        'price_up' => 'Cena rastuća',
        'price_down' => 'Cena padajuća',
        'show' => 'Prikaži',
    ],
    'checkout' => [
        'title' => 'Checkout',
        'form' => [
            'billing_address' => 'Adresa za naplatu',
            'email' => 'E-mail',
            'first_name' => 'Ime',
            'last_name' => 'Prezime',
            'address' => 'Adresa',
            'city' => 'Grad',
            'country' => 'Država',
            'postal_code' => 'Poštanski broj',
            'phone' => 'Telefon',
            'note' => 'Napomena',
        ],
        'your_order' => 'Vaša porudžbina',
        'product' => 'Proizvod',
        'subtotal' => 'Ukupno',
        'new_subtotal' => 'Ukupno nova',
        'total' => 'Sveukupno',
        'tax' => 'Taksa',
        'have_a_coupon' => 'Imate kod za kupon?',
        'discount' => 'Popust',
        'remove_coupon' => 'Poništi kupon',
        'submit' => 'Primeni',
        'credit_or_debit_card' => 'Kreditna ili debitna kartica',
        'terms_and_conditions' => 'Pročitao sam i prihvatam uslove korišćenja',
        'purchase' => 'Kupi',
    ],
    'profile' => [
        'title' => 'Moj nalog',
        'label' => 'Informacije o nalogu',
        'username' => 'Korisničko ime',
        'email' => 'E-mail',
        'role' => 'Dozvola pristupa',
        'created' => 'Napravljen',
        'button' => [
            'edit_profile' => 'Izmeni nalog',
            'check_orders' => 'Porudžbine',
        ],
    ],
    'edit_profile' => [
        'title' => 'Izmena naloga',
        'username' => 'Korisničko ime',
        'email' => 'E-mail',
        'password' => 'Lozinka',
        'password_confirm' => 'Ponovite lozinku',
        'button' => [
            'update' => 'Izmenite',
        ],
    ],
    'orders' => [
        'title' => 'Moje porudžbine',
        'no_order' => 'Nemate ni jednu porudžbinu',
        'list' => [
            'id' => 'Porudžbina ID',
            'product_name' => 'Proizvod',
            'discount' => 'Popust',
            'tax' => 'Taksa',
            'total_price' => 'Ukupna cena',
            'is_shipped' => 'Isporučeno',
            'ordered_on' => 'Naručeno',
            'shipped_yes' => 'Da',
            'shipped_no' => 'Ne',
        ],
    ],
    'order' => [
        'title' => 'Porudžbina',
        'label' => 'Vaša porudžbina',
        'list' => [
            'products' => 'Proizod/i',
            'price' => 'Cena'
        ],
        'subtotal' => 'Ukupno',
        'discount' => 'Popust',
        'total' => 'SVEUKUPNO',
        'tax' => 'Taksa',
        'user' => 'Korisnik',
        'email' => 'E-mail',
        'first_name' => 'Ime',
        'last_name' => 'Prezime',
        'city' => 'Grad',
        'country' => 'Država',
        'postal_code' => 'Poštanski broj',
        'phone' => 'Telefon',
        'name_on_card' => 'Ime na kartici',
        'shipped' => 'Isporučeno',
        'shipped_yes' => 'Da',
        'shipped_no' => 'Ne',
    ],
    'thank_you' => [
        'title' => 'Hvala Vam',
        'message' => '<h1>Hvala vam na kupovini!<br></h1>
                <p>Email sa potvrdom porudžbine je poslat na vašu E-mail adresu.</p>',
        'button' => 'Početna strana',
    ],
    'newsletter_thank_you' => [
        'title' => 'Newsletter',
        'message' => '<h1>Uspešno ste se pretplatili na naš <br> Newsletter </h1>
                <p>Email sa potvrdom je poslat na vašu E-mail adresu.</p>',
        'button' => 'Početna strana',
    ],
    'login' => [
            'registered-users' => 'Registrovani korisnici',
            'registered-users-description' => 'Ukoliko već imate kreiran korisnički nalog kod nas, molimo Vas prijavite se.',
            'new-users' => 'Novi korisnici',
            'new-users-description' => 'Kreiranjem korisničkog računa omogućeno Vam je brže i jednostavnije kupovanje porudžbina, promena podataka, kao i pregled i praćenje statusa Vaših porudžbina i pregled dosadašnjih porudžbina.',
    ],
    'register' => [
            'registered-users' => 'Registrovani korisnici',
            'registered-users-description' => 'Popunite sledeće informacije kako biste kreirali svoj korisnički nalog.',
    ],
];